<template>
  <navbar />
  <!-- Step indicator: Current step is 1 (Complete Info Part 3: Preview) -->
    ...
    <div v-for="(ticket, index) in ticketList" :key="ticket.id" class="card custom-card preview-card">
      ...
      <div class="row preview-row">
        ...
        <div v-if="ticket.type === 'childRun' || ticket.type === 'childWalk'" class="col-md-12">
          <strong>Child ticket certificate:</strong> {{ ticket.proof ? ticket.proof.name : "未上传" }}
        </div>
...
onMounted(() => {
  if (route.query.tickets) {
    try {
      ...
      console.error("解析 tickets 失败", error);
    }
  } else {
